# Respuesta generada — Construir el skill `nuevo-adr`

```yaml
# .claude/skills/nuevo-adr/SKILL.md (sugerido)
name: nuevo-adr
...
1) Plantilla canónica del ADR (replace placeholders):
...
2) Validaciones (procedimiento automatizado):
...
- Verificar que el `context` no sea un placeholder genérico (ej. "contexto").
